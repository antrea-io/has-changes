...
To run tests locally, simply use `./run_tests.sh`. All tests are run with Bash,
and there is no external dependency.

If you modify `check_changes.sh` (which includes most of this Github Action's
logic), you can add new tests as follows:

* Add a new test input file (`test<X>.in`) under `tests/`.
* Add the corresponding golden output file (`test<X>.gold`) under `tests/`. You
  can write it manually, or you can generate it with `/check_changes_test.sh <
  tests/test<X>.in > tests/test<X>.gold 2>&1`, in which case you should check
  that the generated file matches your expectations before committing.
* Run all tests with `./run_tests.sh` to verify the results.
